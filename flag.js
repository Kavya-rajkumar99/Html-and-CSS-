let countriesInfo = [
    {
        name: "India",
        population: " 1,394,975,829",
        region: "Asia",
        flag:
            "https://upload.wikimedia.org/wikipedia/commons/7/7b/India_flag_300.png",
        capital: "New delhi"
    },
    {
        name: "Germany",
        population: "81,770,900",
        region: "Europe",
        flag:
            "https://upload.wikimedia.org/wikipedia/en/thumb/b/ba/Flag_of_Germany.svg/1200px-Flag_of_Germany.svg.png",
        capital: "Berlin"
    },
    {
        name: "United states",
        population: "323,947,000",
        region: "Americas",
        flag:
            "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/Flag_of_the_United_States.svg/640px-Flag_of_the_United_States.svg.png",
        capital: "Washington D.C"
    },
    {
        name: "Brazil",
        population: "206,135,893",
        region: "Americas",
        flag:
            "https://upload.wikimedia.org/wikipedia/commons/0/01/Brazil_flag_300.png",
        capital: "Brasilia"
    },
    {
        name: "Iceland",
        population: "334,300",
        region: "Europe",
        flag:
            "https://upload.wikimedia.org/wikipedia/commons/thumb/c/ce/Flag_of_Iceland.svg/640px-Flag_of_Iceland.svg.png",
        capital: "Reykjavik"
    }
];

const sec = document.createElement("section");
sec.setAttribute("class", "flag-list")
document.body.append(sec);
for (var i in countriesInfo) {
    const divContainer = document.createElement("div");
    divContainer.setAttribute("class", "container");
    const flag = document.createElement("img");
    flag.setAttribute("src", countriesInfo[i].flag);
    flag.setAttribute("class", "flags");
    divContainer.append(flag);
    const divInfo = document.createElement("div");
    divInfo.setAttribute("class", "info");
    divContainer.append(divInfo);
    const heading1 = document.createElement("h2");
    heading1.innerText = countriesInfo[i].name;
    divInfo.append(heading1)
    const pop = document.createElement("p");
    pop.innerText = "Population : " + countriesInfo[i].population;
    divInfo.append(pop);
    const reg = document.createElement("p");
    reg.innerText = "Region : " + countriesInfo[i].region;
    divInfo.append(reg)
    const cap = document.createElement("p");
    cap.innerText = "Capital : " + countriesInfo[i].capital;
    divInfo.append(cap)
    console.log(divContainer)
    sec.append(divContainer)
}

